...
#### 1b. KE-кандидаты (ОБЯЗАТЕЛЬНО при открытии недели)

> Те же данные, что шаг 5e Day Open — но в понедельник их проверяет стратегическая сессия, а не Day Open.

```bash
REPORTS_DIR="$HOME/IWE/{{GOVERNANCE_REPO}}/inbox/extraction-reports"
PENDING=$(grep -l "status: pending-review" "$REPORTS_DIR"/*.md 2>/dev/null | wc -l | tr -d ' ')
echo "KE_PENDING=$PENDING"
```

**Если KE_PENDING > 0** — вывести таблицу файлов (имя | дата | статус) и напомнить:

> ⚠️ **N extraction-report(ов) ожидают разбора.** SLA ≤24ч (DP.SC.004). Запустить `/apply-captures` в отдельной сессии (~15 мин).

Таблицу включить в WeekPlan в секцию «Повестка сессии стратегирования» (чеклист) — чтобы пользователь видел долг явно.

**Если KE_PENDING = 0** — пропустить молча.

#### 1c. Мир (заключения о мире)

> Тот же шаг, что шаг 6 Day Open. В понедельник выполняется в рамках стратегической сессии, а не отдельно.

Feeds из `day-rhythm-config.yaml → news` + WebSearch по темам из активных РП. Субагент Haiku (context isolation):

> Ты — разведчик новостей. Список заголовков + активные РП пользователя. Задача: 2-4 предложения «Что из этого важно для работы этой недели?» Только русский. Только синтез, без перечисления.

Результат → вставить в WeekPlan в секцию «Повестка сессии стратегирования» как чеклист-пункт `[x] Мир — ...`.
...
**Конфликт НЭП (EC-триггер):** если два приоритета из НЭП конфликтуют («и X, и Y — оба важны, но одновременно невозможны») — применить Evaporating Cloud (MIM.M.015):
1. Сформулировать D («делать X») и D' («делать Y») как конкретные действия
2. Найти необходимости B и C (зачем каждое?)
3. Найти общую цель A
4. Вербализовать допущения на каждой стрелке (≥2 на сторону)
5. Найти ложное допущение → injection (что изменить, чтобы снять конфликт)
Результат EC — конкретная инъекция, которая становится кандидатом в РП.
