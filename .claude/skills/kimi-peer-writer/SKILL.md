...
> **Зачем:** консенсус ≠ реализация. Default — реализация в той же сессии (правило 11 CLAUDE.md «финиш > отлог»). Отложение требует одного из явных триггеров.

Извлечь резюме консенсуса (`grep "^CONSENSUS:" "$PEER_FILE" | sed 's/^CONSENSUS: //'`).

**Default = Б (реализовать сейчас).** НЕ задавать пилоту yes/no «фиксируем или реализуем?» — это choice-question «сейчас или потом» (анти-паттерн [feedback_finish_now_no_defer.md](../../../memory/feedback_finish_now_no_defer.md)).

**Сначала проверить триггеры отложения** (если хотя бы один — `IMPLEMENTATION=false`, переход к Шагу 4 с явным обоснованием пилоту):

1. **Реализация требует нового РП** (новый scope, не покрытый текущим РП — другой Pack, другой домен, другой бюджет ×2-×3).
2. **Требуется доменное решение пилота** (порог, имя, форматный выбор), которое не было обсуждено в сессии.
3. **Требуется ArchGate** (новое архитектурное решение системного уровня).
4. **Контекст полностью переключился** (другая часть системы; реализация требует другого набора файлов и нового framing).
5. **Реализация уже выполнена внутри сессии** (по ходу обсуждения — turn-замечание реализовано сразу, отдельный pipeline избыточен).
6. **Пилот явно сказал «зафиксируй»** в текущей сессии.

Если ни один триггер не сработал — анонс пилоту (НЕ вопрос):
```
Decision Gate: реализую сейчас (default per правило 11). Триггеров отложения нет.
Файлы: <list>. Метод: Edit/Write напрямую. Переход к Шагу 3.6.
```

Если триггер сработал — анонс пилоту (НЕ вопрос):
```
Decision Gate: отложение по триггеру [N: <название>]. Обоснование: <...>.
Перехожу к Шагу 4 (report.md + commit). Реализация — отдельный РП/фаза: <предложение>.
```

Записать выбор в `meta.yaml` (Bash sed):
```yaml
implementation_pipeline: false | true
defer_trigger: "<номер и название>" # если IMPLEMENTATION=false
```

- **`IMPLEMENTATION=true`** (default) → перейти к Шагу 3.6.
- **`IMPLEMENTATION=false`** (по триггеру) → перейти к Шагу 4.
...
Напиши <SESSION_DIR>/report-draft.md строго по схеме ниже.
